...
class ProductTag(models.Model):
    ...
    def cron_import_product_tag(self):
        ...
        for rec in all_instances:
            if rec:
                self.env['product.tag'].import_product_tag(rec)
        ...
        while page > 0:
            ...
            if data.status_code == 200:
                ...
                for rec in parsed_data:
                    ...
                    if existing_tag:
                        existing_tag.write(dict_value)
                    else:
                        self.env['product.tag'].create(dict_value)
    ...
    def cron_export_product_tag(self):
        ...
        for rec in all_instances:
            if rec:
                self.env['product.tag'].export_selected_product_tag(rec)
    ...
    def export_selected_product_tag(self, instance_id):
        ...
        selected_records = self.env['product.tag'].browse(selected_ids)
        all_records = self.env['product.tag'].search([])
        if selected_records:
