...
class WooInstance(models.Model):
    ...
    def instance_total_orders(self):
        ...
        view = self.env.ref('pragtech_woo_commerce.action_sale_order_woo').read()[0]
    ...
    def instance_products(self):
        product_data = {}
        self._cr.execute("""select count(id) as total_count from product_template where
                        is_exported = True and woo_instance_id = %s""" % self.id)
    ...
    def instance_customers(self):
        customer_data = {}
        self._cr.execute("""select id from res_partner where is_exported = True and woo_instance_id = %s"""
                         % self.id)
        result = self._cr.dictfetchall()
    ...
    def instance_taxes(self):
        tax_data = {}
        self._cr.execute("""select id from account_tax where is_exported = True and woo_instance_id = %s"""
                         % self.id)
        result = self._cr.dictfetchall()
        tax_ids = [data.get('tax_id') for data in result]
        view = self.env.ref('account.action_tax_form').read()[0]
        action = self.prepare_action(view, [('is_exported', '=', True), ('woo_instance_id', '=', self.id)])
        tax_data.update({'tax_count': len(tax_ids), 'tax_action': action})
        return tax_data

    def instance_product_categ(self):
        category_data = {}
        self._cr.execute("""select id from product_category where is_exported = True and woo_instance_id = %s"""
                         % self.id)
        result = self._cr.dictfetchall()
        category_ids = [data.get('category_id') for data in result]
        view = self.env.ref('product.product_category_action_form').read()[0]
        action = self.prepare_action(view, [('is_exported', '=', True), ('woo_instance_id', '=', self.id)])
        category_data.update({'category_count': len(category_ids), 'category_action': action})
        return category_data

    def instance_product_attribute(self):
        attribute_data = {}
        self._cr.execute("""select id from product_attribute where is_exported = True and woo_instance_id = %s"""
                         % self.id)
        result = self._cr.dictfetchall()
        attribute_ids = [data.get('attribute_id') for data in result]
        view = self.env.ref('product.attribute_action').read()[0]
        action = self.prepare_action(view, [('is_exported', '=', True), ('woo_instance_id', '=', self.id)])
        attribute_data.update({'attribute_count': len(attribute_ids), 'attribute_action': action})
