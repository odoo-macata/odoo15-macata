...
class ProductCategory(models.Model):
    ...
    def export_selected_category(self, instance_id):
        ...
        selected_records = self.env['product.category'].browse(selected_ids)
        all_records = self.env['product.category'].search([])
        if selected_records:
                ...
                if parsed_data:
                    if parsed_data.get('product_categories'):
                        ...
                        for category in parsed_data.get('product_categories'):
                            ...
                            product_category = self.env['product.category'].search([('woo_id', '=', category.get('id'))],
                                                                                   limit=1)
                            dict_e = {}
                            ''' This is used to update woo_id of a product category, this
                            will avoid duplication of product while syncing product category.
                            '''
                            product_category_without_woo_id = self.env['product.category'].search(
                            ...
                            if category.get('parent'):
                                parent = self.env['product.category'].search([('woo_id', '=', category.get('parent'))],
                                                                             limit=1)
